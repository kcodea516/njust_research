...
最终复验结果目录为 `Vehicle5_FINAL_YOLO26l_960_bce_val`。该目录使用 `YOLO26l 960 + 默认 BCE` 的 `best.pt` 重新验证得到，归一化混淆矩阵与训练阶段最优结果一致，说明最终权重复验稳定。车辆检测和五类车型分类模块当前可以阶段性收尾，后续进入车牌检测识别模块。
